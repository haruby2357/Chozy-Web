...
export default function CommentRow({
  ...
  const previewReplies = useMemo(() => {
    ...
    if (showAllReplies) return children;
    return children.slice(0, DEFAULT_PREVIEW);
  }, [children, hasChildren, showAllReplies]);

  const hiddenCount = Math.max(children.length - DEFAULT_PREVIEW, 0);
  ...
  }, [hasChildren, isChild, previewReplies.length, showAllReplies]);
