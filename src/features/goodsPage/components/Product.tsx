...
export default function Product({
  ...
  const openProduct = () => {
    if (isSoldOut) return; // 품절이면 클릭 막기
    ...
    <div className={`flex flex-col gap-2 ${s.w}`}>
      ...
      <div className="relative w-full flex justify-center">
        <div className={`relative w-full ${s.h}`}>
          ...
          {/* 품절 오버레이(이미지 위) - 클릭 막기는 onClick에서 처리 */}
          {isSoldOut && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="inline-flex px-[16px] py-[12px] justify-center items-center gap-[10px] rounded-[4px] bg-[rgba(87,87,87,0.90)]">
                <span className="text-white font-pretendard text-[14px] font-normal leading-[140%]">
                  품절된 상품이에요.
                </span>
              </div>
            </div>
